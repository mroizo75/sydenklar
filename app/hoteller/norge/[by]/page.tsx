...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const cityName = findCityBySlug(COUNTRY_NAME, by)
  if (!cityName) return {}

  const total = getHotelCountByCity(COUNTRY_NAME, cityName)
  return generateDestinationMetadata({
    title: `Hoteller i ${cityName} – Sammenlign ${total} hoteller | Sydenklar`,
    description: `Finn og bestill hotell i ${cityName}, Norge. Sammenlign ${total} hoteller med de beste prisene på Sydenklar.`,
    path: `/hoteller/norge/${by}`,
  })
}

export default async function NorgeByPage({ params }: Props) {
  const { by } = await params
  const cityName = findCityBySlug(COUNTRY_NAME, by)
  if (!cityName) notFound()

  const hotels = getHotelsByCity(COUNTRY_NAME, cityName, 48)
  const total = getHotelCountByCity(COUNTRY_NAME, cityName)
  ...
  const breadcrumb = buildBreadcrumbSchema([
    ...
    { name: cityName, url: `${SITE_URL}/hoteller/norge/${by}` },
  ])

  const itemList = buildItemListSchema(hotels, `/hoteller/norge/${by}`, slugify)
      ...
      <div className="bg-[var(--deep)] pt-20 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-sm text-white/50 mb-6 flex-wrap">
            ...
            <span>/</span>
            <Link href="/hoteller/norge" className="hover:text-white transition-colors">Norge</Link>
            <span>/</span>
          ...
          <p className="text-white/60 text-base">
            {total.toLocaleString('nb-NO')} hoteller i {cityName}, Norge
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {hotels.map(hotel => {
              ...
              return (
                <Link
                  key={hotel.id}
                  href={`/hoteller/norge/${by}/${hotelSlug}`}
